  ...
  return (
    <div className="container mx-auto px-4">
      <div className="text-center my-10">
        <h1 className="text-5xl font-semibold">KC’s eminent university tie-ups</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4">
        {[Image, image, Img, img, Gill, Monash, Queens, BRISTOL].map((src, index) => (
          <div
            key={index}
            className={`flex justify-center items-center p-8 border-gray-300 
            ${index % 4 !== 3 ? "border-r" : ""} 
            ${index < 4 ? "border-b" : ""}`}
          >
            <img src={src} alt={`University ${index}`} className="w-40 h-auto" />
          </div>
        ))}
      </div>
